...
// Tests of the test utilities themselves. They do not need the developer package, so they run in the `unitTest`
// suite and are excluded from the suites that test the package.
val unitTestPattern = "com.microej.test.util.*"
...
testing {
    suites {
        ...
        // Unit tests of the test utilities. This suite does not depend on `assemble`, so it runs without building
        // the developer package: `./gradlew unitTest`.
        val unitTest by registering(JvmTestSuite::class) {
            targets {
                all {
                    testTask.configure {
                        filter { includeTestsMatching(unitTestPattern) }
                    }
                }
            }
        }
        ...
        withType<JvmTestSuite> {
            ...
            // The unit tests run in the `unitTest` suite only, so every other suite skips them.
            targets {
                all {
                    testTask.configure {
                        if (name != "unitTest") {
                            filter { excludeTestsMatching(unitTestPattern) }
                        }
                    }
                }
            }
