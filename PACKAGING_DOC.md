...
- The tests of the test utilities themselves (package `com.microej.test.util`) do not test the package, so they run in
  the separate `unitTest` suite, which does not depend on `assemble`: `./gradlew unitTest` takes seconds instead of
  building the package first. The split is a filter on that package name, applied in `packaging/build.gradle.kts`, so a
  test class added to `com.microej.test.util` runs in `unitTest` only.

### How to Test the Build and Run Cases

All the build and run tests follow the same pattern: start a task of the packaged sources with a Gradle build, then
assert on its outcome. The outcome takes three forms, checked as the test needs: the exit code of the build, the files
the build produced, and the traces written to the build output. Two test utilities carry this pattern:
`GradleBuildRunner` starts the builds, and `StringOutputBuffer` records their output and waits for traces in it.
...
The examples above use `startBuild`, which runs the build to its end and returns its exit code. It fits any task that
terminates on its own, like `buildFeature` or `buildExecutable`.

A task that keeps running never lets `startBuild` return: `runOnSimulator`, or `runOnDevice` when the build stays
attached to the running executable, like on the Linux x86 VEE Port. Start it with `startAsynchronousBuild`: the method
returns as soon as the build process is started, and the test does its assertions on the output while the build runs.
On most real hardware, `runOnDevice` terminates once the executable is deployed, so it runs with `startBuild` and the
device traces are read from the serial connection (see [Using another VEE Port](#using-another-vee-port)).
...
can then be tested with the wait methods described below.
...
The `start*Build` methods never close the writer: the creator of the buffer owns it, and closes it with the
try-with-resources block. A build therefore never closes a buffer that another source still writes to (a serial
connection, a later build). A closed buffer silently drops everything written afterwards, and logs one warning when it
does, so do not close a buffer that a running build still feeds.

When the test does not assert on the output of a build, there is no need to accumulate it in a buffer: pass
`new OutputStreamWriter(System.out)` as the writer. The output is echoed to the standard output and nothing is kept in
memory. A typical case is a build that deploys a feature while the assertions run on the kernel output.

#### Waiting for Traces in the Output

Two methods wait for the traces of a running build. Both block until the trace is written or the timeout elapses:

- `waitUntilContains(trace)` — wait for a trace. Returns `false` when the trace does not appear.
- `waitUntilMatches(pattern)` — wait for a trace **and read a value out of it**, for example a port or an IP address.
  Returns the `MatchResult` of the occurrence awaited, so its capture groups give the value of that occurrence, or
  `null` when the pattern does not appear.

For example:

    private static final Pattern PORT_PATTERN =
            Pattern.compile("AppConnect listening on \\(http\\) port:\\s*(\\d+)\\R");

    @Test
    public void testKernelRunOnDevice() throws Exception {
        try (StringOutputBuffer output = newOutputBuffer()) {
            try {
                buildRunner.startAsynchronousBuild(srcDir, output.getWriter(), ":kernel-linux-x86:runOnDevice");

                // Wait for a trace
                assertTrue(output.waitUntilContains("INFO: Starting Kernel"));

                // Wait for a trace and read the port out of it
                MatchResult match = output.waitUntilMatches(PORT_PATTERN);
                assertNotNull(match);
...
Each call waits for the **next** occurrence of the trace, so two calls with the same trace wait for two distinct
occurrences. This is what tells apart several applications printing the same trace, and it means the calls must follow
the order in which the traces are written.

A test that checks traces from several outputs (a running kernel, later builds building and deploying features, a
serial connection) simply shares one `StringOutputBuffer`: pass the same writer to every build. The buffer then records
one chronological transcript, but the interleaving between two outputs is not guaranteed.

When the order of the traces is not guaranteed, for example when two threads print their traces independently or when
several outputs share the buffer, pass `true` as the `fromStart` argument of the wait to search the whole output, from
its beginning:

    assertTrue(output.waitUntilContains("Application running: app", true));

Such a wait leaves the search position untouched, so it finds a trace already matched or written before the previous
match, and the ordered waits that follow it are not affected. Both methods share the same position, so they can be
mixed. The method `getContent()` is never affected: it always returns the whole output.

Both methods have an overload taking a timeout in milliseconds. Without it, the timeout comes from the system property
`string.output.wait.timeout`, set in the `targetTestProperties()` function of `build.gradle.kts`.

Two rules when writing a pattern:

- **Anchor a pattern that ends with a quantifier** on the text that follows the value, normally the line break.
  Otherwise it can match a half-written trace and give a truncated value:

      Pattern.compile("port:\\s*(\\d+)\\R"); // matches 4001
      Pattern.compile("port:\\s*(\\d+)");    // may match 40

- A match must be shorter than 8192 characters, which is far above the length of a trace (about 70 lines of a typical
  build log). A longer match can be missed, and the warning logged when the wait fails mentions this limit.

#### Destroying Asynchronous Builds
